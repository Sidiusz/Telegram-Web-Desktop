...
test('desktop notification popup follows Telegram toast geometry and has no dead settings flags', () => {
    ...
    assert.match(notif, /if\(futureHeight>window\.innerHeight\+1\)await resizeWindow\(futureHeight\)/);
    assert.match(notif, /stack\.appendChild\(el\)/);
    assert.match(notif, /const enterOffset=newHeight\+GAP/);
    assert.match(notif, /node\.style\.transform='translateY\('\+dy\+'px\)'/);
    assert.match(notif, /evicted=oldEls\[0\]/);
    assert.match(notif, /async function removeCardNow\(id\)/);
    assert.match(notif, /await resizeWindow\(contentHeight\(Array\.from\(stack\.children\)\.slice\(-MAX_CARDS\)\)\)/);
