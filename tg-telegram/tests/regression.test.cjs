...
test('virtual feed is local, source-driven and reuses Telegram cached messages', () => {
    ...
    assert.match(feed, /forward-title-container/);
    assert.match(feed, /Avatar forward-avatar size-micro peer-color-/);
    ...
    assert.match(core, /_twd-feed-view_\{[^}]*background:transparent!important/);
