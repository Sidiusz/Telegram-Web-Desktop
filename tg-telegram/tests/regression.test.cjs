...
test('virtual feed is local, source-driven and reuses Telegram cached messages', () => {
    ...
    assert.match(feed, /message-content','peer-color-'\+peerColor,'has-action-button','has-shadow','has-solid-background','has-appendix/);
    assert.doesNotMatch(feed, /forward-title-container|Переслано от|Forwarded from/);
    ...
    assert.match(feed, /_twd-feed-source-title_/);
    assert.match(feed, /Avatar size-micro _twd-feed-source-avatar_ peer-color-/);
    assert.doesNotMatch(feed, /forwarded-message/);
    ...
    assert.match(core, /_twd-feed-message_\{[^}]*padding-inline-start:44px!important/);
