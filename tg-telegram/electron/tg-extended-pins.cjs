...
const uniq=a=>Array.isArray(a)?[...new Set(a.map(String).filter(x=>/^-?\\d{1,24}$/.test(x)))]:[];
const clean=a=>uniq(a).slice(0,15);
...
const api={enabled:${flag},get(id,key,native){const x=load(),a=account(x,id),v=a.orders[String(key)];return Array.isArray(v)?clean(v):clean(native)},set(id,key,v){const x=load(),a=account(x,id),k=String(key),n=clean(v);a.orders[k]=n;save(x);persist(id,k,n);return n},server(id,key,v){const x=load(),a=account(x,id);a.server[String(key)]=clean(v);save(x)},mergeList(id,key,v,record){if(!Array.isArray(v))return v;const raw=clean(v);if(record)this.server(id,key,raw);if(!this.enabled)return raw;const x=load(),a=account(x,id),local=a.orders[String(key)];if(!Array.isArray(local))return raw;return clean([...local,...raw.filter(x=>!local.includes(String(x)))])},mergeFolder(id,key,f){if(!f)return f;const raw=clean(f.pinnedChatIds);this.server(id,key,raw);if(!this.enabled)return f;const x=load(),a=account(x,id),local=a.orders[String(key)];if(!Array.isArray(local))return f;const pins=clean([...local,...raw.filter(x=>!local.includes(String(x)))]),inc=uniq([...pins,...(f.includedChatIds||[])]);return{...f,pinnedChatIds:pins,includedChatIds:inc}},cap(s){return s&&s.users&&s.users.byId&&s.users.byId[s.currentUserId]&&s.users.byId[s.currentUserId].isPremium===true?10:5},limit(s){return this.cap(s)+5},serverAfter(before,next,id,pin,cap){let out=clean(before).slice(0,cap);if(pin){if(out.includes(id)||out.length>=cap)return out;return clean([id,...out]).slice(0,cap)}if(!out.includes(id))return out;out=out.filter(x=>x!==id);for(const q of clean(next)){if(out.length>=cap)break;if(!out.includes(q))out.push(q)}return out},toggle(s,a,p,rpc){if(!this.enabled)return false;const id=String(p&&p.id||'');if(!id)return false;const fid=Number(p&&p.folderId||0),chat=s.chats&&s.chats.byId&&s.chats.byId[id],key=fid?String(fid):(chat&&chat.folderId===1?'1':'0'),native=fid?(s.chatFolders&&s.chatFolders.byId&&s.chatFolders.byId[fid]&&s.chatFolders.byId[fid].pinnedChatIds||[]):(s.chats&&s.chats.orderedPinnedIds&&s.chats.orderedPinnedIds[key==='1'?'archived':'active']||[]),cur=this.get(s.currentUserId,key,native),i=cur.indexOf(id);if(i<0&&cur.length>=this.limit(s)){try{globalThis.__twdExtendedPinsLimit&&globalThis.__twdExtendedPinsLimit(this.limit(s))}catch(_){}return true}const next=i>=0?cur.filter(x=>x!==id):[id,...cur];const x=load(),rec=account(x,s.currentUserId);if(!Array.isArray(rec.server[key]))rec.server[key]=clean(native).slice(0,this.cap(s));rec.orders[key]=clean(next);save(x);persist(s.currentUserId,key,next);if(fid){const f=s.chatFolders&&s.chatFolders.byId&&s.chatFolders.byId[fid];if(f){const inc=uniq([...next,...(f.includedChatIds||[])]),local={...f,pinnedChatIds:clean(next),includedChatIds:inc};try{a.apiUpdate({'@type':'updateChatFolder',id:fid,folder:local})}catch(_){}const before=clean(rec.server[key]||[]),server=this.serverAfter(before,next,id,i<0,this.cap(s));rec.server[key]=server;save(x);if(JSON.stringify(server)!==JSON.stringify(before)){try{Promise.resolve(rpc('editChatFolder',{id:fid,folderUpdate:{...f,pinnedChatIds:server,includedChatIds:inc}})).catch(()=>{})}catch(_){}}}}else{try{a.apiUpdate({'@type':'updateChatPinned',id,isPinned:i<0})}catch(_){}const before=clean(rec.server[key]||[]),after=this.serverAfter(before,next,id,i<0,this.cap(s));rec.server[key]=after;save(x);if(typeof rpc==='function'&&JSON.stringify(after)!==JSON.stringify(before)){(async()=>{try{for(const q of before.filter(x=>!after.includes(x))){const c=s.chats.byId[q];if(c)await rpc('toggleChatPinned',{chat:c,shouldBePinned:false})}for(const q of after.filter(x=>!before.includes(x)).reverse()){const c=s.chats.byId[q];if(c)await rpc('toggleChatPinned',{chat:c,shouldBePinned:true})}}catch(_){}})()}}return true},restore(){const x=load(),jobs=[];for(const[id,v]of Object.entries(x.accounts||{})){for(const[k]of Object.entries(v.orders||{})){if(Array.isArray(v.server&&v.server[k]))jobs.push(persist(id,k,v.server[k]))}}return Promise.all(jobs)},setEnabled(v){this.enabled=v===true;return this.enabled?Promise.resolve():this.restore()}};globalThis.__twdExtendedPins=api;})();`;
}
